/* =========================================================
   admin.js — Logic trang quản trị Không gian VR 360°
   Đọc/ghi public/data/spaces.json + ảnh/audio qua GitHub API.
   Không có server: "đăng nhập" là lớp che nhẹ; quyền ghi thật = GitHub token.
   ---------------------------------------------------------
   ⚠️ QUY ƯỚC ĐƯỜNG DẪN (sau khi chuyển sang Astro):
   • Đường dẫn LƯU trong spaces.json giữ nguyên dạng cũ:  assets/panos/x.jpg
     (site đọc tương đối từ web-root — KHÔNG đổi)
   • Đường dẫn GHI qua GitHub API phải thêm 'public/':    public/assets/panos/x.jpg
     (file tĩnh nằm trong public/ của dự án Astro) → dùng hàm repoPath()
   ========================================================= */
...
// URL hiển thị/preview: root-absolute (site phục vụ tại gốc tên miền)
const PREVIEW = '/';
...
// path nhận dạng WEB (assets/...) — tự thêm 'public/' khi gọi API
async function ghDeletePathBestEffort(path){
  try{ const c = await ghGetContent(repoPath(path)); if(c && c.sha) await ghDeleteFile(repoPath(path), 'admin: xoá file '+path, c.sha); }catch(e){ /* bỏ qua */ }
}
...
$('#filePicker').addEventListener('change', async (e)=>{
  ...
  try{
    ...
    if(p.kind==='audio'){
      ...
      await ghPutFile(repoPath(apath), b64, 'admin: tải audio '+apath);
    ...
    if(p.kind==='exhibit-img'){
      ...
      await ghPutFile(repoPath(ipath), base64, 'admin: tải ảnh hiện vật '+ipath);
    ...
    await ghPutFile(repoPath(fullPath), full.base64, 'admin: tải ảnh 360° '+fullPath);
